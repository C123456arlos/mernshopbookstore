        ...
        <header className='max-w-screen-2xl mx-auto px-4 py-6'>
            <nav className='flex justify-between items-center'>
                <div className='flex items-center md:gap-16 gap-4'>
                    <Link to='/'><HiMiniBars3CenterLeft className='size-6'></HiMiniBars3CenterLeft></Link>
                    <div className='relative sm:w-72 w-40 space-x-1'>
                        ...
                        <input type='text' placeholder='start' className='bg-[#eaeaea] w-full py-1 md:px-8 px-6 rounded-md
                                        ...
                                        <ul className='py-2'>
                                            {navigation.map((item) => (
                                                <li key={item.name} onClick={() => setIsDropdownOpen(false)}>
                                                    <Link to={item.href} className='block px-4 py-2 text-ms hover:bg-gray-100'>{item.name}</Link>
                                                </li>
                                            ))}
                                            <li>
                                                <button onClick={handleLogOut}
                                                    className='block w-full text-left px-4 py-2 text-ms hover:bg-gray-100'>logout</button>
                                            </li>
                                        </ul>
                                    </div>
                                )
